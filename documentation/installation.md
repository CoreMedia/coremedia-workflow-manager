...
Go to [Release](https://github.com/CoreMedia/coremedia-workflow-manager/releases) and download the version that matches your CMCC release version.
The ZIP file provides the Maven workspace of the plugin.

## Activate the Plugin

The coremedia-workflow-manager is a plugin for studio-server and studio-client.
...
2. Build the `coremedia-workflow-manager`
  ...
  2. Run `pnpm install && pnpm -r run build && pnpm -r run package` in the folder `studio-client`.
     ...
     Checkpoint: A zip file exists in `studio-client/apps/main/coremedia-workflow-manager/build`.
3. Create a directory for studio-server plugins, e.g. `/tmp/studio-server-plugins`,
   and copy `coremedia-workflow-manager/studio-server/target/studio-server.coremedia-workflow-management-plugin-<version>.zip`
   into that directory.
...
5. Start the studio client with an additional property `-DadditionalPackagesDirs=/.../coremedia-workflow-manager/studio-client/target/app`
